...
const AddTrip = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${API}/api/trips`, payload, {
  ...
  return (
    <div className="pt-20 px-4 max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-center">Add New Trip</h2>
          ...
          {tripData.dayWisePlan.length > 0 && (
            <ul className="mt-4 list-disc list-inside text-sm text-gray-700 space-y-1">
              {tripData.dayWisePlan.map((plan, index) => (
                <li key={index}>
                  <strong>Day {plan.day}:</strong> {plan.title}
                </li>
              ))}
            </ul>
          )}
